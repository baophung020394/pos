...
interface SelectCityBranchProps {
...
const SelectCityBranch: React.FC<SelectCityBranchProps> = ({ endpoint, name, label, control, startIcon, endIcon, className }) => {
                ...
                <Select {...field} label={label} fullWidth className={className} startAdornment={renderAdornment('start')} endAdornment={renderAdornment('end')}>
                    {data &&
                        data?.data.map((option: any, index: number) => (
                            <MenuItem key={index} value={option.cityId}>
                                {option.cityName}
                            </MenuItem>
                        ))}
                </Select>
...
export default SelectCityBranch;
